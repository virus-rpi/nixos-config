...
let
  ...
  '';
  kittyConf = import ../terminal/kitty.nix{inherit pkgs;};
...
writeText "sway-config" (
  ''
    ...
    bindsym $mod+Shift+Return exec ${kitty}/bin/kitty -c ${kittyConf}
    ...
    bindcode 123 exec --no-startup-id ${config.services.pulseaudio.package}/bin/pactl set-sink-volume @DEFAULT_SINK@ +5% #increase sound volume
    bindcode 122 exec --no-startup-id ${config.services.pulseaudio.package}/bin/pactl set-sink-volume @DEFAULT_SINK@ -5% #decrease sound volume
    bindcode 121 exec --no-startup-id ${config.services.pulseaudio.package}/bin/pactl set-sink-mute @DEFAULT_SINK@ toggle # mute sound
